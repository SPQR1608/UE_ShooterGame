...
{
	if (IsDead() || IsHealthFool()) return false;

	SetHealth(Health + HealtAmount);
	return true;
}

bool USTUHealthComponent::IsHealthFool() const
{
...
void USTUHealthComponent::ApplyDamage(float Damage, AController* InstigatedBy)
{
	...
	/*
	* TODO Make no friendly damage, Make new Damagetype USTUWeaponDamageType
	* if(Cast<USTUFireDamageType>(DamageType))
	*/
	if (InstigatedBy)
	{
		...
		const auto AreEnemies = STUUtils::AreEnemies(Player->Controller, InstigatedBy);
...
void USTUHealthComponent::ReportDamageEvent(float Damage, AController* InstigatedBy)
{
	...
	UE_LOG(HealthComponentLog, Display, TEXT("Report damage: %f, name: %s"), Damage, *InstigatedBy->GetName());
